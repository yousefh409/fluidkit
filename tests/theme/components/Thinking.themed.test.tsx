...
/**
 * Theme wiring for `Thinking`. No module mocks: unmocked in jsdom, Motion's
...
// Thinking's derived accent share is 16% (src/theme/derive.ts).
const ACCENT_MIX =
  /^color-mix\(in srgb, (?:#2D6A4F|rgb\(45, 106, 79\)) 16%, transparent\)$/;

describe("Thinking (themed)", () => {
  ...
  it("explicit tint beats the theme accent", () => {
    const { container } = render(
      <FluidThemeProvider theme={{ accent: "#2D6A4F" }}>
        <Thinking tint="rgba(200, 220, 255, 0.4)" />
      </FluidThemeProvider>
    );
  ...
  it("explicit material and color beat the theme's", () => {
    const { container } = render(
      ...
      >
        <Thinking material="flat" color="#ff0000" />
      </FluidThemeProvider>
    );
