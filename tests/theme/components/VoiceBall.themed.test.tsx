...
/**
 * Theme wiring for `VoiceBall`. No module mocks: unmocked in jsdom, Motion's
...
// VoiceBall's derived accent share is 16% (src/theme/derive.ts).
const ACCENT_MIX =
  /^color-mix\(in srgb, (?:#2D6A4F|rgb\(45, 106, 79\)) 16%, transparent\)$/;

describe("VoiceBall (themed)", () => {
  ...
  it("explicit tint beats the theme accent", () => {
    const { container } = render(
      <FluidThemeProvider theme={{ accent: "#2D6A4F" }}>
        <VoiceBall tint="rgba(200, 220, 255, 0.4)" />
      </FluidThemeProvider>
    );
  ...
  it("explicit material and color beat the theme's", () => {
    const { container } = render(
      ...
      >
        <VoiceBall material="flat" color="#ff0000" />
      </FluidThemeProvider>
    );
  ...
  it("theme intensity drives specular opacity; explicit intensity wins", () => {
    // VoiceBall's static ball specular paints at 0.5 × volume.
    ...
    for (const opacity of specularOpacities(themedOnly.container)) {
      expect(opacity).toBeCloseTo(0.25, 12);
    }
    const explicit = render(
      <FluidThemeProvider theme={{ intensity: 0.5 }}>
        <VoiceBall intensity={0.9} />
      </FluidThemeProvider>
    );
    for (const opacity of specularOpacities(explicit.container)) {
      expect(opacity).toBeCloseTo(0.45, 12);
    }
  });

  it("renders the untinted default fill with no provider mounted", () => {
    const { container } = render(<VoiceBall />);
